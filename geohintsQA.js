...
const run = async () => {
    ...
    for (const continent of Array.from(continents)) {
        ...
        for (const bollard of Array.from(bollards)) {
            const imgSrc = bollard.querySelector(`img.${IMG_CLASS}`)?.getAttribute("src") || bollard.querySelector(`img.${IMG_CLASS_2}`)?.getAttribute("src");;
            const imgAlt = imgSrc?.replace("Sources/Signs/Numbers/", "").replace("http://www.geohints.com/", "").replace(`Sources/${URL_ROUTE}/`, "");
            ...
            const answer = aElement.outerHTML + " - " + googleLocation;
